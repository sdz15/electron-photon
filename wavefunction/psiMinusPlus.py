...
def psi_minus_plus(t_ph, s_ph, t_el, s_el):
    x = psimp(s_ph - t_ph, s_el + t_el)

    r_j1 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 1) * (t_el - s_el + sigma) * r_psimp(s_ph - t_ph, sigma)
    i_j1 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 1) * (t_el - s_el + sigma) * i_psimp(s_ph - t_ph, sigma)
    y = -(omega / 2) * (quad(r_j1, s_el - t_el, s_el + t_el)[0] + quad(i_j1, s_el - t_el, s_el + t_el)[0])

    r_j0 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 0) * r_psimm(s_ph - t_ph, sigma)
    i_j0 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 0) * i_psimm(s_ph - t_ph, sigma)
