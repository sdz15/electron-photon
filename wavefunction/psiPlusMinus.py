...
def psi_plus_minus(t_ph, s_ph, t_el, s_el):
    x = psipm(s_ph + t_ph, s_el - t_el)

    r_j1 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 1) * (t_el + s_el - sigma) * r_psipm(s_ph + t_ph, sigma)
    i_j1 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 1) * (t_el + s_el - sigma) * i_psipm(s_ph + t_ph, sigma)
    y = -(omega / 2) * (quad(r_j1, s_el - t_el, s_el + t_el)[0] + quad(i_j1, s_el - t_el, s_el + t_el)[0])

    r_j0 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 0) * r_psipp(s_ph + t_ph, sigma)
    i_j0 = lambda sigma: besselfun(besselterm(t_el, s_el, sigma), 0) * i_psipp(s_ph + t_ph, sigma)
